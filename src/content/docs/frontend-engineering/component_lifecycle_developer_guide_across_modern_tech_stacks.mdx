...
*   **React (with Hooks):**
    ...
    *   **Key Hooks:** `useState` (state), `useEffect` (side effects, covers mounting, updating, and unmounting via its dependency array and cleanup function), `useLayoutEffect` (similar to `useEffect` but fires synchronously after all DOM mutations).
    *   **Stateless:** A functional component that only takes props and renders UI, without `useState` or `useEffect`.
